...
class Data_precess:
    ...
    def read(self): # weibo_essay
        ...
        sql ="select id,comment_content from comment where label = '{}'".format(filmname.film)
    ...
    def data_to_csv(self,Dict = {}):
        ...
        df = pd.DataFrame({
            'id':df['id'],
            'comment':df['comment'],
            'content_type':self.sentiment_score(list(df['comment']))
        })
        df.to_csv('comment.csv',encoding='utf-8',columns=['id','comment','content_type']) #行标
    ...
    def prehandle(self,Dict = {}):
        '''
        得到分词结果，词性，和相对于该段话的位置
        '''
        df = pd.read_csv('comment.csv',encoding='utf-8')

        comment = df['comment'].head(COUNT)


        # 去除去除英文、数字等
        # 由于评论主要为京东美的电热水器的评论，因此去除这些词语
        strinfo = re.compile('[a-zA-Z0-9’!"#$%&\'（）() *+,-./:：;；|<=>?@，—。?★、…【】《》？“”‘’！[\\]^_`{|}~]+'
                             '|鹿晗|上海堡垒|电影|粉丝|说|演员')
        comment = comment.apply(lambda x: strinfo.sub('', x))

        # 分词
        worker = lambda s: [(x.word, x.flag) for x in psg.cut(s)]  # 自定义简单分词函数
        seg_word = comment.apply(worker)
        ...
        content_type = [[x] * y for x, y in zip(list(df['content_type']), list(n_word))]
        ...
        # 删除停用词
        stop_path = open("NLPIR_stopwords.txt", 'r', encoding='UTF-8')
        ...
        plt.axis('off')
        # plt.show()
        plt.savefig('film_cloud.png')

        '''
        其中 inded_Word 是改词在改段话中的相对位置
...
3                 2    帅      a           0
        ...
        '''
        result.to_csv("jieba_film.csv", index=False, encoding='utf-8')
        return result

    def sentiment_analysis(self):
        # 代码12-6 匹配情感词
        word = pd.read_csv("jieba_film.csv")
...
if __name__ == '__main__':
    ...
    # process.prehandle(temp) #前期数据处理 分词 词性标注 词云
